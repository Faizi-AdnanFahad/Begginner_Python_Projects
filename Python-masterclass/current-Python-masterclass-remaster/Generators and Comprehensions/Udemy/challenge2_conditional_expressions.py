...
# COMPREHENSION
# 1
leads_to_forest = [locations[num] for num in exits if 5 in exits[num].values()]
print(leads_to_forest)


# 2
leads_to_forest_tuple = [(num, locations[num]) for num in exits if 5 in exits[num].values()]
print(leads_to_forest_tuple)

list_normal_forest = []
for x in exits:
    if 5 in exits[x].values():
        list_normal_forest.append(locations[x])
print(list_normal_forest)

list_of_pair_tuple = []
for x in exits:
    if 5 in exits[x].values():
        list_of_pair_tuple.append((x, locations[x]))
print(list_of_pair_tuple)

print("----------------------------------------------------------------------------------------------------------")

for loc in locations:
    leads_to_forest_tuple = [(num, locations[num]) for num in exits if loc in exits[num].values()]
    print(f"Locations leading to {loc}: \t", leads_to_forest_tuple)
